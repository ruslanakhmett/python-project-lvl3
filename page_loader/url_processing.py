...
def get_chunk(response):
    try:
        ...
        )  # елдит итерируемую последовательность
...
def is_local_resource(
    page_url, link
):  # поверяем ссылки на то что они являются ссылками на локальные ресурсы типа /search/opensearch.xml либо полными ссылками с указанием того же домена полностью
